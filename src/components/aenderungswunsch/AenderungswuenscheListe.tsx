...
/**
 * Die Tabelle `aenderungswuensche` fehlt in den generierten Supabase-Types.
 * Im Baukasten uebliches Cast-Muster (siehe melde-feature-einbauen.md, A3).
 * ACHTUNG: `supabase.storage.from("aenderungswuensche")` NICHT casten -
 * Storage ist untypisiert und heisst nur zufaellig gleich.
 */
...
export function AenderungswuenscheListe() {
  ...
  const setzen = async (id: string, felder: Partial<Wunsch>) => {
    const { error } = await aenderungTable().update(felder as never).eq("id", id);
