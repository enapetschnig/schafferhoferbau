-- Weiterleitung der Aenderungswuensche ins CRM
--
-- Die App SCHICKT, das CRM sammelt nur ein. Dadurch braucht das CRM keine
-- Supabase-Schluessel dieser App. Bilder und Sprachnachrichten bleiben im
-- privaten Bucket; das CRM holt bei Bedarf eine signierte URL bei der Edge
-- Function `wunsch-datei` - beides ueber dasselbe Geheimnis.
--
-- WICHTIG: Der Trigger ist inaktiv, solange cockpit_verbindung leer ist.
-- Diese Migration laesst sich also gefahrlos ausrollen; scharf geschaltet
-- wird spaeter mit einem INSERT (siehe B4 der Anleitung).
...
CREATE TABLE IF NOT EXISTS public.cockpit_verbindung (
  ...
  app_key   text NOT NULL DEFAULT 'schafferhoferbau'
);
...
-- INSERT und die relevanten UPDATEs: `text` aendert sich, wenn die Abschrift
-- der Sprachnachricht nachtraegt; status/antwort, wenn der Wunsch bearbeitet
